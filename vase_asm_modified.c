...
//org property-maxincrement.c file in cubic directory with applying constraint simplification (Final step in applying VASE)
#include <stdio.h>
#include <stdlib.h>
...
void some_function() {
    klee_id("Entering some_function");
...
#define error_msg(msg) { \
    char debug_buf[256]; \
    sprintf(debug_buf, "\x1B[31m%s !\x1B[0m\n", msg); \
    klee_id(debug_buf); \
}
...
//instead of strcmp() since strcmp is not being handled correctly
int string_equals(const char *str1, const char *str2) {
    if (str1 == NULL || str2 == NULL) {
        return 0; // Strings are not equal if either is NULL
    }
    while (*str1 && *str2) {
        if (*str1 != *str2) {
...
void test_string_equals() {
    printf("Test 1: %d\n", string_equals("hello", "hello")); // Should print 1
    printf("Test 2: %d\n", string_equals("hello", "world")); // Should print 0
    printf("Test 3: %d\n", string_equals("test", NULL)); // Should print 0
    printf("Test 4: %d\n", string_equals("", "")); // Should print 1
}
...
//modified: string_equals instead of strcmp
void __VERIFIER_nondet_u32(u32 *variable, char *name, u32 min_value, u32 max_value) {
#ifdef KLEE
    klee_make_symbolic(variable, sizeof(u32), name);
printf("Comparing %s and %s: Result = %d\n", name, "which_rtt", string_equals(name, "which_rtt"));

    if (string_equals(name, "which_rtt")) {
        klee_assume(*variable == 0);
	printf("Applying VASE constraint: which_rtt = 0\n");
    } else if (string_equals(name, "ssthresh")) {
        klee_assume(*variable == 1);
	printf("Applying VASE constraint: ssthresh = 1\n");
    } else if (string_equals(name, "epoch_start")) {
        klee_assume(*variable == 1);
	printf("Applying VASE constraint: epoch_start = 1\n");
    } else {
        klee_assume(*variable >= min_value);
        klee_assume(*variable <= max_value);
	printf("Default constraint: %s within [%u, %u]\n", name, min_value, max_value);
    }
#else
    if (string_equals(name, "which_rtt") == 0) {
        *variable = 0;
    } else if (string_equals(name, "ssthresh") == 0) {
        *variable = 1;
    } else if (string_equals(name, "epoch_start") == 0) {
        *variable = 1;
    } else {
        *variable = max_value;
    }
