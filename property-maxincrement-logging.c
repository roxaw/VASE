...
void log_variables(const char *location, struct tcp_sock *tp, struct bictcp *ca)
{
    printf("[LOG] %s: snd_cwnd=%u, snd_ssthresh=%u, epoch_start=%u, delay_min=%u, bic_K=%u, bic_target=%u, cnt=%u\n",
           location, tp->snd_cwnd, tp->snd_ssthresh, ca->epoch_start, ca->delay_min, ca->bic_K, ca->bic_target, ca->cnt);
}
...
int main(int argc, char *argv[])
{
    ...
    u32 MIN_BIC_K; // min_K must be greater than zero
    u32 MAX_BIC_K;
    ...
    cubictcp_register(); // necessary to initialize cubic parameters
    ...
    // Log initial state
    log_variables("Initialization", tp, ca);
    ...
    if (ENABLE_AGG) {
        ...
        log_variables("After bictcp_cong_avoid_agg", tp, ca);
        ...
        for (i = 0; i < cwnd_at_rtt; i++) {
            ...
            log_variables("During loop in bictcp_cong_avoid", tp, ca);
        }
        diff_result = __VERIFIER_assert("[Verifier1] Condition tp->snd_cwnd <= 1.5*sym_init_cwnd", tp->snd_cwnd <= (cwnd_at_rtt * 3 / 2));
    ...
    // Log final state
    log_variables("Final State", tp, ca);
